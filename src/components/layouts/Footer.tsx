...
export function Footer() {
  ...
  return (
    <footer className="bg-card border-t border-border/60 text-foreground pt-20 pb-10">
      <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
        ...
        <div className="flex flex-col space-y-6">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span
                className="text-[7px] tracking-widest uppercase text-accent/85 mt-1.5 leading-none"
        ...
        {/* Explore Links */}
        <div>
          <h3 className="text-xs uppercase tracking-widest text-accent font-semibold mb-6" style={{ letterSpacing: "0.15em" }}>
            Explore
          </h3>
          <ul className="space-y-4 text-xs font-sans">
            {NAV_LINKS.map((link) => (
              <li key={link.path}>
                <Link href={link.path} className="text-muted hover:text-foreground transition-colors duration-300">
                  {link.name}
                </Link>
              </li>
            ))}
        ...
        <div className="flex flex-col space-y-6">
          <div>
            <h3 className="text-xs uppercase tracking-widest text-accent font-semibold mb-4" style={{ letterSpacing: "0.15em" }}>
              The Gazette
            </h3>
            ...
            <form onSubmit={handleSubscribe} className="relative flex items-center border-b border-border/40 focus-within:border-accent transition-colors duration-300 pb-2">
              <input
                type="email"
                ...
                className="bg-transparent text-xs w-full text-foreground placeholder-muted/50 outline-none pr-8 font-sans"
              />
              <button type="submit" aria-label="Subscribe" className="absolute right-0 text-foreground hover:text-accent transition-colors duration-300">
                <ArrowRight size={16} />
              </button>
            </form>
            {submitted && (
              <p className="text-[10px] text-accent mt-2 font-sans">Welcome to our exclusive list.</p>
            )}
          </div>
          <div className="text-[10px] text-muted space-y-1 font-sans leading-relaxed pt-2">
            <p className="font-semibold text-foreground/95">HQ: {address}</p>
            <p>Tel: {phone}</p>
            <p>Mail: {emailVal}</p>
          </div>
        </div>
      ...
      {/* Bottom Copyright */}
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-8 border-t border-border/30 flex flex-col md:flex-row items-center justify-between gap-4 text-[10px] text-muted font-sans">
